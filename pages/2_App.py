    ...
    if st.session_state.selected_book_id == "UPLOAD":
        ...
        if uploaded_file:
            with st.status(f"Indexing {uploaded_file.name}...", expanded=True) as status:
                ...
                try:
                    pages_text = PDFProcessor.extract_pages(temp_pdf_path)
                    if pages_text:
                        bid = str(uuid.uuid4())
                        db_pages = []
                        for pt in pages_text:
                            db_pages.append({'page_number': pt.page_number, 'raw_text': pt.raw_text, 'cleaned_text': preprocessor.clean(pt.raw_text)})
                        repo.create_book(bid, uploaded_file.name, uploaded_file.name.replace('.pdf', ''), len(db_pages))
                        repo.save_pages(bid, db_pages)
                        index_manager.ensure_index(bid)
        ...
        if not current_book:
            ...
            # Show book meta and a delete option in main view
            st.markdown("---")
